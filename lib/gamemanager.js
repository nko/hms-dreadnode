...
GameManager.prototype.placeShips = function(client, locations) {
  ...
  console.log("Sending readyack message to " + manager.connections[client.sessionId].playa.username);
  manager.connections[client.sessionId].playa.ready = true;
  var readyAckMsg = {type: "readyack", msg: "Yup, you're ready"};
  client.send(JSON.stringify(readyAckMsg));
